...
-- Количество треков вошедших в альбомы 2019-2020 годов (1987-1992 годов включительно)
...
-- Все исполнители, которые не выпустили альбомы в 2020 году (в 2011 году, у меня древние альбомы)
...
-- Название сборников в которых присутствует исполнитель 'Разные артисты, A. Makarevich'
...
-- Название альбомов, в которых присутствуют исполнители более одного жанра
...
-- Исполнитель, написавший самый короткий трек (теоретически таких исполнителей может быт несколько)
...
INNER JOIN track ON album.album_id = track.album_field
GROUP BY album.name_album
HAVING count(track.album_field) = (SELECT min(quantity_tracks_in_album) FROM (SELECT count(track.album_field) AS quantity_tracks_in_album FROM album
														 INNER JOIN track ON album.album_id = track.album_field
